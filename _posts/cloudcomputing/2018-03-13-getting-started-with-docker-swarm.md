...
---
여러 Node로 구성된 Docker Host 집단을 손쉽게 관리하기 위한 기술 중 하나로
...
쓰이는 부분 중 하나가 용어를 어디까지 순화할 것인가에 대한 고민이라면...)
...
우리식(이라고는 하지만 한자를 빌겠지) 표현을 찾아야 할지... 혹은 그것이
얼마나 통용이 될지 항상 고민을 하게 된다.

---

잡설은 접고,  
오늘의 글, 그리고 이 뒤를 이을 몇 편의 글은 Docker에 대한 묶음글에 딸린
작은 묶음글로, Docker Host 집단을 마치 큰 하나의 가상의 Docker 처럼 쓸
수 있도록 해주는 관리 소프트웨어인 Docker Swarm에 대해 정리하려고 한다.
(벌써 이 묶음글오 아홉번째 글이 되었다.)
...
뜻하기도 한다. 이를 명확하게 구분하여 부르기 위해 기존의 Swarm 프로젝트는
[Docker Swarm standalone]으로 부른다. 이 글에서 Swarm이라고 칭할 때에는
Docker Engine에 결합된 클러스터 관리 기능을 말하는 것이다.

참고로, Docker Swarm standalone은 아직 Deprecate 계획이 없다고 하며, API
호환성을 바탕으로 아직도 활용이 가능하다.

Docker Engine을 Swarm Cluster에 참여하게 하면, Docker Engine은 Cluster의
일부로써 관리가 가능한데, 이 경우를 특별히 칭할 때 **Docker Swarm mode**
라고 부르기도 한다. Docker가 Swarm mode로 실행되게 되면, 단위 Engine이 아닌
Cluster 단위로 Engine 관리와 Container 관리가 가능하게 된다. 이 글에서는,
이 Swarm Mode를 켜는 방법에 대하여 위의 목차와 같은 순서로 정리한다.


{:#feature-of-docker-swarm}
## Docker Swarm의 기능

Swarm mode를 사용하게 되면, Docker의 Container 관리 기능에 더해 다음과
같은 Cluster 관리 기능 또는 이와 관련된 특징을 확보하게 된다. (이 글에서는
이 기능의 맥을 파악하는 정도로 정리하겠지만, 그것 만으로도 긴 글이 될 것
같다.)
...
{:.boxed.definition}
> Container
> : 사용자가 목적하는 프로그램의 독립적 실행을 위한 격리된 공간
>
> Task
...
>
> Service
> : 사용자가 Docker Swarm에게 업무를 할당하는 논리적 단위로,
> 결과적으로 여러 Task의 집합이 됨
>
> Dockerized Host 또는 Host
> : Docker Engine이 탑재된 Virtual Machine이나 Baremetal을 뜻한다.
> 
> Swarm Node 또는 Node
> : Swarm mode로 동작하는 Docker Engine이 탑대된 Host를 뜻한다.
> 하지만 오랜 버릇으로 인해, 이전 글에서는 Host와 같은 의미로 사용하기도 했다.
>
> Manager Node 또는 Manager
> : Swarm Node 중에서 Cluster 관리 역할을 수행하는 Node (겸임 가능)
>
> Worker Node 또는 Worker
> : Swarm Node 중에서 Container를 실행하여 실제 일을 처리하는 Node (겸임 가능)


사용자가 Docker Client를 이용하여 Service를 만들게 되면,

1. Manager Node는 Service를 Task로 세분하고 복사본의 배치 계획을 세움
1. Manager Node는 세워진 계획에 따라 Task를 Worker Node에 배치함
1. Worker Node는 Task를 받아 각각에 대한 Container를 생성하여 실행시킴

일반적인 VM과는 달리, 일단 Task가 Worker에게 할당되면 이 Task는 다른 Node로
이전할 수 없고 해당 Node에서 생성, 삭제, 장애의 생명주기를 마치게 된다.
...
$ docker-machine ls
NAME   ACTIVE  DRIVER     STATE    URL                       SWARM  DOCKER
dev01  *       softlayer  Running  tcp://169.56.125.222:2376        v18.02.0-ce
dev02  -       softlayer  Running  tcp://169.56.125.216:2376        v18.02.0-ce
$ 
```
